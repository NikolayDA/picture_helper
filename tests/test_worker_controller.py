...
def test_mesh_build_cancel_and_drain_with_real_thread(qapp, controller, monkeypatch):
    ...
    assert started_second
    assert controller.mesh_build_thread is not first_thread
    assert controller.mesh_build_worker is not first_worker
    assert first_thread in controller._mesh_build_draining
    assert first_worker._cancelled is True

    # Der zweite Build ist nicht gegated und laeuft uneingeschraenkt durch.
    _drain(qapp, lambda: controller.mesh_build_thread is None)
    assert [gen for _mesh, gen in results] == [2]
    assert errors == []

    # Jetzt den ersten, drainenden Build freigeben – er muss ueber die
    # Draining-Liste sauber abgeschlossen werden, ohne die (bereits auf None
    # stehenden) aktuellen Handles erneut anzufassen.
    release_first.set()
    _drain(qapp, lambda: controller._mesh_build_draining == [])
