...
from bgremover.canvas_events import apply_pan_delta, cursor_for_crop_hit, should_start_pan
...
class ImageCanvas(QGraphicsView):
    ...
    def _handle_crop_press(self, btn: Qt.MouseButton, sp: QPointF) -> bool:
        ...
        if btn == Qt.MouseButton.LeftButton:
            ...
            if corner >= 0:
                ...
                self.setCursor(cursor_for_crop_hit(corner, inside=False))
    ...
    def _handle_crop_move(self, sp: QPointF) -> bool:
        ...
        if self._crop_overlay is not None:
            corner = self._crop_overlay.corner_hit(sp.x(), sp.y())
            inside = self._crop_overlay.inside(sp.x(), sp.y())
            self.setCursor(cursor_for_crop_hit(corner, inside))
            return True
        return False
    ...
    def _handle_panning_move(self, event) -> bool:
        ...
        apply_pan_delta(hbar, vbar, delta.x(), delta.y())
        return True
