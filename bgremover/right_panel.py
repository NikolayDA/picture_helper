...
class _RightPanelBuilder:
    ...
    def build(self) -> RightPanel:
        ...
        refs: WidgetRefs = {}
        refs.update(self._add_tab(
            tabs,
            *SelectionTab(self._actions).build(),
            title="Auswahl",
            icon_name="clear_sel",
            tooltip="Auswahl – Zauberstab, Pinsel, Radiergummi",
        ))
        refs.update(self._add_tab(
            tabs,
            *BackgroundTab(self._actions).build(),
            title="Hintergrund",
            icon_name="bg",
            tooltip="Hintergrund – Entfernen, Farbe ersetzen",
        ))
        refs.update(self._add_tab(
            tabs,
            *TransformTab(self._actions).build(),
            title="Drehen/Spiegeln",
            icon_name="transparency",
            tooltip="Transform – Drehen, Spiegeln",
        ))
        refs.update(self._add_tab(
            tabs,
            *ShapeTab(self._actions).build(),
            title="Form",
            icon_name="form",
            tooltip="Form & Zuschnitt – Ecken abrunden, Format-Auswahl",
        ))
    ...
    def _add_tab(
        ...
        *,
        ...
        tabs.setTabIcon(idx, make_tool_icon(icon_name, _TAB_ICON_PX))
