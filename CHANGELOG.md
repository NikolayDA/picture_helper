...
- **EufyMake-Export schreibt die Projekt-Auflösung als PNG-`pHYs` (#689, #691).**
  `color_motif.png`, `height_map.png` und `gloss_mask.png` tragen jetzt die aus
  der physischen Projektgröße abgeleiteten X- und Y-DPI getrennt als
  `pHYs`-Chunk (ganzzahlige Pixel pro Meter). eufyMake Studio 4.2.2 übernimmt
  diesen Wert beobachtet als Startgröße; ohne `pHYs` startete es bisher mit
  72 dpi, ein 1200-px-Motiv wurde so 423 mm groß. Ohne gesetzte physische
  Größe entsteht weiterhin kein Chunk; Pixeldaten und `manifest.json` sind
  unverändert, die Warnung zur physischen Größe beschreibt den neuen Stand.
- **Qt auf 6.11 gehoben; die aarch64-Artefakte verlangen jetzt Debian 13
  (#994).** Der Pin `PyQt6-Qt6` lag auf 6.7.3, weil neuere Wheels für aarch64
  glibc 2.39 voraussetzen. Raspberry Pi OS baut inzwischen Debian 13 „Trixie"
  (glibc 2.41), womit diese Begründung nur noch einen Altstand schützte. Der
  Sprung schließt rund zehn Qt-Advisories, die 6.7.3 betrafen, darunter die
  beiden Qt-SVG-Befunde CVE-2025-10728 und CVE-2025-10729; der Use-after-free
  darunter fiel nicht unter die frühere, ausdrücklich auf reine Abstürze
...
- **3D-Vorschau meldete „bereit", obwohl die Umgebung keinen Frame rendern
  ...
  Kontext, GL-2.1-Funktionssatz. Auf einem Raspberry Pi 5 (Debian 13 „Trixie",
...
- **„Höhenkarte aus Bild erzeugen" prominent an der Spitze von Schritt 5.**
  ...
  „Beschaffen"-Kasten kartenlos an der Spitze der rechten Spalte von
  Schritt 5 – noch vor der Ebenen-Karte und dem 3D-Abschnitt; „Graustufe
  importieren…" folgt direkt darunter (weiterhin nur im Experten-Modus).
...
- **Auswirkung:** Reines Sicherheits-Patch-Release. Es behebt ausschließlich
  die oben genannte ICNS-Schwachstelle (CVE-2025-5683) in den „Zuletzt
  geöffnet"-Miniaturansichten; keine neuen Funktionen, keine Änderung an
  Bild-, Projekt- oder Exportverhalten.
- **Betroffene Anwender:innen:** Wer die „Zuletzt geöffnet"-Liste nutzt und
  Dateien aus nicht vertrauenswürdigen Quellen öffnet – ein präpariertes,
  unter einem zuvor geöffneten Pfad ausgetauschtes ICNS konnte bislang Qts
  eigenen, ungefilterten Bild-Decoder erreichen. Wer nur eigene Dateien
  öffnet, ist nicht akut gefährdet, sollte aber dennoch aktualisieren.
- **Upgrade-Relevanz:** Empfohlen für alle Nutzer:innen. Kein
  Migrationsschritt nötig – Projektdateien (`.bgrproj`), Exportformate und
  Einstellungen bleiben unverändert kompatibel; ein Downgrade auf 2.7.2 ist
  ebenso ohne Datenänderung möglich.
- **Unterstützte Plattformen:** macOS arm64 (`.dmg`), Linux x86_64 und Linux
  arm64 (je `.AppImage` und `.deb`), Python ≥ 3.10. Alle Artefakte bündeln das
  KI-Backend (`-ai`-Suffix). Die macOS-App ist ad-hoc signiert, nicht mit
  Developer ID notarisiert – beim Erststart daher Rechtsklick → „Öffnen".
- **Bekannte Einschränkungen:** Keine über 2.7.2 hinausgehenden neuen
  Einschränkungen.
...
- **3D-Reliefvorschau: GPU-Puffer/VAO sammelten sich bei wiederholtem
  Mesh-Upload an (PR #676).** Jeder Wechsel zwischen 2D- und 3D-Ansicht bzw.
  jede Wiederanzeige eines bereits gebauten Mesh legte neue OpenGL-Puffer und
  ein neues Vertex-Array-Objekt an, ohne die vorherigen freizugeben – über
  eine längere Sitzung wuchs so der GPU-Speicherbedarf der 3D-Vorschau
...
- **Zukunftsversionen des Projektformats werden strikt abgewiesen (#588).**
  Ein Format ab v3 wird vor jeder Verarbeitung unbekannter Felder oder
  Payloads mit einer übersetzten Aktualisierungsaufforderung beendet. Das
  aktive Projekt und die Datei bleiben unverändert; dadurch kann ein älterer
  Stand keine zukünftigen Daten unbemerkt als v2 zurückschreiben.
