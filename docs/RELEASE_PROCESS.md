...
| `actions/checkout` bzw. ein Action-Download scheitert mit HTTP 429 (`codeload.github.com` lehnt die Heim-IP der Self-hosted Runner ab) | Trifft den **ersten** Schritt jedes Self-hosted-Jobs, also vor dem Preflight und außerhalb der Reichweite des Watchdogs: rein lesenden Action-Archiv-Cache des Runners einrichten bzw. prüfen ([`RELEASE_AUTOMATION.md`](RELEASE_AUTOMATION.md) §2.3), danach den betroffenen Lauf mit denselben gebundenen Inputs wiederholen — Abnahme mit derselben Kandidaten-Run-ID, Post-Release-Nachweis mit derselben `run_id` und `predecessor_tag`. Der Kandidat bleibt gültig, weil die Behebung außerhalb des ausgeführten Kandidatenstands liegt | fehlende Plattform als `PASS` markieren, den Fehlversuch nicht protokollieren oder `CHECK_FAILED` aus demselben Lauf als Release-Befund werten |
