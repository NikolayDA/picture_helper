...
- **Qt actualizado a 6.11; los artefactos aarch64 requieren ahora Debian 13
  (#994).** El pin `PyQt6-Qt6` estaba en 6.7.3 porque las ruedas más nuevas
  exigen glibc 2.39 en aarch64. Raspberry Pi OS ya compila Debian 13 «Trixie»
  (glibc 2.41), de modo que ese argumento solo protegía un estado antiguo. El
  salto cierra unas diez alertas de Qt que afectaban a 6.7.3, entre ellas los
  hallazgos de Qt SVG CVE-2025-10728 y CVE-2025-10729; el use-after-free no
  quedaba cubierto por la aceptación de riesgo anterior, basada expresamente en
...
- **El modo Estándar y la navegación entre pasos no descartaban una vista
  previa de optimización de altura activa (#839).** Al cambiar del modo
  Experto al Estándar o al salir del paso Relieve con la tarjeta
  "Optimizar" desplegada y una vista previa en vivo sin aplicar, la vista
  previa seguía visible en el lienzo aunque los controles de Aplicar/
...
- **AppImage Linux con IA: corregido en origen el bloqueo por spawn recursivo
  (#633).** `python-appimage` establece `sys.executable` en el AppImage externo,
  por lo que `multiprocessing` reiniciaba la aplicación completa para cada
  proceso hijo de IA. Los procesos de IA usan ahora para `spawn` el intérprete
  incluido en `$APPDIR/usr/bin`. El autotest y la inferencia crean así un único
  hijo; el timeout y la protección contra fork bombs siguen activos.
...
- **Auditoría posterior 3D: capacidad lazy, estado de UI coherente y
  ...
  QSettings y paneles reconstruidos; Restablecer recupera todos los parámetros;
  y un contexto GL perdido vuelve a cargar el mesh de CPU.

- **La AppImage de Linux vuelve a iniciarse; compatibilidad aarch64
  restaurada (#595).** Pruebas manuales en hardware real Raspberry Pi 5
  revelaron dos errores de empaquetado: la AppImage carecía de su propio
  punto de entrada `AppRun` e invocaba el intérprete Python empaquetado sin
  argumentos, terminando en una consola interactiva en lugar de la GUI (ya
  corregido mediante una receta `entrypoint.sh` dedicada); además, las
  versiones fijadas de PyQt6/PyQt6-Qt6 requerían en aarch64 una glibc más
  reciente (2.39) que la que trae el actual Raspberry Pi OS/Debian 12
  "bookworm" (2.36), fallando allí con un error de carga `GLIBC`.
  PyQt6/PyQt6-Qt6 vuelven a la última versión portable para aarch64
  (6.7.1/6.7.3).

- **Staging de plugins de Qt: los plugins de plataforma en staging vuelven a
  encontrar sus bibliotecas Qt6 (#595, hallazgo posterior).** Más pruebas en
  Raspberry Pi tras el arreglo de glibc revelaron un tercer fallo de inicio:
  el directorio de staging endurecido para plugins de plataforma
  (`qt_plugins.py`) colocaba los archivos de forma plana, rompiendo el
  `RUNPATH` compilado (`$ORIGIN/../../lib`) de `libqxcb.so` — el cargador
  recurría a una Qt6 del sistema posiblemente incompatible (fallo
  `undefined symbol` en sistemas con su propia Qt6, p. ej. Raspberry Pi OS).
  El staging ahora refleja exactamente el anidamiento original
  (`Qt6/plugins/platforms`) y añade un enlace simbólico `Qt6/lib` seguro y
  vinculado a la versión que apunta al directorio de bibliotecas real.

- **Modelo híbrido de análisis de seguridad: CodeQL automático, Codex solo
  manual (#551).** CodeQL ahora se ejecuta como una configuración avanzada
  versionada (`.github/workflows/codeql.yml`), analizando Python
  automáticamente en cada push/PR a `main` y semanalmente, aportando una
  cobertura SAST determinista y nativa de GitHub sin depender de cuota de
  API externa. El Codex Security Scan existente ahora se ejecuta
  exclusivamente mediante `workflow_dispatch` manual; la ruta de cadencia
  de 14 días anterior (fecha ancla, job `cadence`) y sus interruptores de
  activación/desactivación se eliminan por ser redundantes para un
  disparador puramente manual. El prompt, el esquema JSON, la carga de
  artefactos, la validación de hallazgos, la sincronización deduplicada de
  issues y la separación de permisos (`issues: write` solo en el job de
  sincronización posterior) permanecen sin cambios. `SECURITY.md` describe
  ahora el panorama completo y actualmente activo de comprobaciones de
  seguridad; la decisión de arquitectura está documentada como un ADR
...
- **Las versiones futuras del formato de proyecto se rechazan estrictamente
  (#588).** Un formato v3 o posterior se detiene antes de procesar campos o
  payloads desconocidos, con una indicación traducida para actualizar la
  aplicación. El proyecto activo y el archivo no cambian, evitando que una
  versión antigua reescriba silenciosamente datos futuros como v2.
...
- **Se elevó el pin de setuptools contra una nueva CVE.** `setuptools` se
  ...
  ya fijadas.
- **Nombres de artefactos de release ahora inequívocos por
  plataforma/dispositivo (#584).** Las cinco descargas de release
  (AppImage/`.deb` para Linux x86_64 y Linux/Raspberry Pi aarch64, `.dmg`
  para macOS arm64) ahora se llaman
  `BgRemover-X.Y.Z-<etiqueta-de-plataforma>[-ai].<extensión>` en lugar de la
  arquitectura `uname` sin más – p. ej.
  `BgRemover-2.6.0-linux-raspberrypi-arm64-ai.AppImage`. Antes el `.deb` de
  Linux/Raspberry Pi y el `.dmg` de macOS llevaban ambos la misma etiqueta
  `arm64` y solo se distinguían por la extensión; el sufijo `-ai` además
  deja visible que cada artefacto – incluido el build de Raspberry Pi –
  incluye la misma eliminación de fondo por IA integrada que el build de
  macOS.
