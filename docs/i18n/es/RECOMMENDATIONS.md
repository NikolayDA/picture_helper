...
- **O2 🟢 — App de Linux / empaquetado.** ✅ Hecho (PR 5 + PR 6): **AppImage** portable más un segundo formato **.deb** (desktop file, icono, metadatos AppStream); un **workflow de release** construye ambos para **x86_64 y aarch64/Raspberry Pi OS** en runners nativos y los adjunta al release. Los smoke tests mantienen metadatos y workflow consistentes — reduce la barrera de entrada análogo al bundle `.app` de macOS.
...
- **PR 5 — Base de empaquetado Linux.** ✅ Hecho. AppImage como artefacto objetivo: `packaging/linux` con `.desktop` Freedesktop, metainfo AppStream y script de build `python-appimage`; id de app `de.bgremover.app` (igual al bundle de macOS), `app.py` usa `setDesktopFileName`; smoke test autocontenido (consistencia desktop/AppStream/pyproject).
- **PR 6 — Ampliación del empaquetado Linux.** ✅ Hecho. Segundo formato `.deb` (envuelve la AppImage → apt install + integración en el menú), variante aarch64/Raspberry Pi OS y un workflow de release de GitHub Actions (AppImage + `.deb` para x86_64 y aarch64, adjuntos al release); el smoke test construye realmente un `.deb` y verifica el workflow.
