...
- **#583/#586 (2026-07-15, same day):** the v2.6.0 scope freeze/version/CHANGELOG curation (#583) and the 16-bit HEIGHT ADR (#586) are completed and closed; #584 and #591 are now unblocked as a result.
...
Live state: **18** open issues – the same snapshot as earlier the same day, but with two sub-issues closed in the meantime (**#583**, **#586**) and updated owner comments on **#245**/**#551**. The comment check found: #245 and #551 each received a new comment dated 2026-07-15 – the owner used them to lock in the #551 strategic decision (hybrid model: CodeQL automatic + Codex manual only) and narrowed both issues' scope accordingly; both issue bodies are already updated, so no additional comment from me was needed. All other 16 issues remain without comments.
...
- **3D relief preview** (#582 → #591 → #592 → #593 → #594 → #595): #591 is now unblocked since #586 is closed – it can start right away in parallel with the 16-bit model work; #582 remains the largest effort block this round regardless.
...
| [#584](https://github.com/NikolayDA/picture_helper/issues/584) | Release 2.6.0: candidate gate, five artifacts | 🟠 High | 🟠 High | Sonnet 5 · high | **Ready for PR** – #583 is closed, no open dependency left; the five artifact smokes fit well-suited parallel workflow orchestration. |
...
| [#591](https://github.com/NikolayDA/picture_helper/issues/591) | [3D] ADR/UX contract: renderer backend, fallback, budgets | 🟡 Medium | 🟠 High | Opus 4.8 · high | **Ready for PR** – #586 is closed, no open dependency left; pure ADR/UX concept work, can start in parallel with the 16-bit model implementation. |
| [#592](https://github.com/NikolayDA/picture_helper/issues/592) | [3D] Qt-free geometry/normal/decimation pipeline | 🟡 Medium | 🟠 High (very large) | Opus 4.8 · high | **Blocked** – waits on #591. |
...
| [#582](https://github.com/NikolayDA/picture_helper/issues/582) | [Epic] Real 3D relief preview | 🟡 Medium | 🟠 High (very large) | – (tracking epic) | **In progress** – runs via #591→…→#595; #591 now ready to start (#586 closed). |
...
1. **#584** — tackle the v2.6.0 candidate gate now: #583 is closed, no open dependency left; locks in the release candidate across all five artifacts.
2. **#591** — start the 3D ADR/UX contract now rather than after #586 is "accepted": #586 is closed, #591 blocks #592–595 and is explicitly allowed to run in parallel with the 16-bit model implementation (#587+).
...
*Drift note:* re-check the live open-issue count before every future update instead of carrying it forward – within this same day, two sub-issues (#583, #586) were closed and unblocked two downstream issues (#584, #591); a plain timestamp snapshot would have missed that.
