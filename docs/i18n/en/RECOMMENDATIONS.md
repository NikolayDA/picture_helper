...
| [#1053](https://github.com/NikolayDA/picture_helper/issues/1053) | `make doctor` contradicts the SessionStart hook's editable contract (#1031): FAIL in every web session | 🟡 Medium (the diagnostic command points the wrong way in web sessions; `make install-test` destroys the hook state) | 🟡 Medium (mode switch in `check_test_env.py`, reuse the link rule from `check_install_provenance.py`, new tests, three doc locations) | Sonnet, medium | Ready for PR – independent; the owner decides the shape of the mode switch |
