...
**EufyMake #681/#687–#691:** PR #948 is merged, and #689 preparation now provides 36 individual fixtures plus a real four-file export bundle. Separate X/Y DPI, deliberately conflicting manifest/`pHYs` values, and pixel-exact COLOR/HEIGHT/GLOSS landmarks are covered by automated checks. Two Studio 4.2.2 import paths are documented without printing. #687 remains at 16/18 criteria: I-06 still needs the real Studio observation, followed by the closeout review. Full 16-bit use, size priority, real mm dimensions, grayscale-to-mm mapping, and gloss intensity remain hardware questions in #688–#690.
...
| [#689](https://github.com/NikolayDA/picture_helper/issues/689) | Validate mm/DPI, target size, positioning contract | 🟠 High (print size/registration) | 🔴 High (physical measurements, control motifs) | – (no agent; needs real hardware) | In progress – repository groundwork is complete: 36 fixtures, a real export manifest, X/Y DPI, and COLOR/HEIGHT/GLOSS registration are verified. Controlled Studio imports, priority/rounding observations, and physical measurements remain open |
...
3. Import **#689** in Studio under controlled conditions and then—only after device/material
   approval—measure it together with the remainder of #687, #688, and #690.
