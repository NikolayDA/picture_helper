...
| [#1010](https://github.com/NikolayDA/picture_helper/issues/1010) | Measure the `frameSwapped` render proof on macOS (cocoa) | 🟠 High (unmeasured; a false alarm would hit `MACOS-ARM-DMG-01`) | 🟡 Medium (measurement on the Mac runner + doc follow-up) | Sonnet, small + hardware | Log the counters per event loop pass on `cocoa`, update the ADR addendum and PACKAGING_SMOKE; decide on the checklist only afterwards |
