...
process scripts – verified, bundled as follow-up issue #943, and fixed
with this state.
...
**EufyMake #681/#687–#691:** the 34 fixtures, protocol templates, and approved test governance are now reflected in the issues. Two controlled import paths passed in Studio 4.2.2; the remaining import matrix and physical E1 measurements are still open. #687 is at 16/18 criteria; only I-06 (folder/manifest) and the closeout review after the real tests remain. For the separate Spot UV path, the manufacturer-backed hypothesis is black = gloss and white = no gloss. Full 16-bit use, `pHYs` priority, grayscale-to-mm mapping, and gloss intensity remain hardware questions in #688–#690.
...
| [#688](https://github.com/NikolayDA/picture_helper/issues/688) | Validate HEIGHT bit depth/semantics on real hardware | 🟠 High (directly affects relief height) | 🔴 High (physical printer, fixtures, measurement log) | – (no agent; needs real EufyMake hardware) | Blocked (external) – repository preparation is complete with 34 fixtures, matched I-08 landmarks, the constant-RGB alpha cell I-13, and a trusted pre-import report; two controlled Studio 4.2.2 import paths are documented. The remaining import matrix and safe physical E1 measurements are open |
