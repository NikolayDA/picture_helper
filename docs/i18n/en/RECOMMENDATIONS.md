...
**EufyMake #681/#687–#691:** PR #948 is merged, and #689 now provides 36 individual fixtures plus a real four-file export bundle. Separate X/Y DPI, conflicting manifest/`pHYs` values, and pixel-exact COLOR/HEIGHT/GLOSS landmarks are covered by automated checks. Studio 4.2.2 confirms the 72-DPI fallback, per-axis `pHYs` priority, unsupported JSON image import, manual size priority, rotation, and single-image crop. I-06 is therefore observed; #687 is at 17/18 criteria and only awaits the closeout review after the real tests. Full 16-bit use, physical print dimensions, grayscale-to-mm mapping, and gloss intensity remain hardware questions in #688–#690.
...
| [#690](https://github.com/NikolayDA/picture_helper/issues/690) | Validate gloss/clear-coat semantics | 🟡 Medium (gloss is already flagged "experimental" in code) | 🔴 High (physical prints, material consumption) | – (no agent; needs real hardware) | Blocked (external) + groundwork partial – alpha/coverage and pixel-exact COLOR/HEIGHT/GLOSS landmarks exist; additional gloss dimensions, intensity/polarity cells, and real preview/print observations remain open |
...
3. After device/material approval, perform the remaining physical measurements for **#689**
   together with the remainder of #687, #688, and #690; #689's Studio import portion is documented.
