...
- **Versioned EufyMake target profile (#691).** Planner, validator, dialog, and
  ...
  remedies. The dialog shows the profile version and separate X/Y DPI; the
  manifest includes a profile snapshot and BgRemover version. 16-bit is the
...
- **Qt raised to 6.11; the aarch64 artifacts now require Debian 13 (#994).**
  ...
  older systems instead of failing at startup. x86_64 rises too, from glibc
...
- **Standard mode and step navigation did not discard an active
  height-optimize preview (#839).** Switching from Expert to Standard mode
  or leaving the Relief step while the "Optimize" card was expanded with
  an uncommitted live preview left the preview visible on the canvas even
  though the Apply/Discard controls were no longer reachable — Save/
...
- **Linux AppImage starts again; aarch64 compatibility restored (#595).**
  Manual testing on real Raspberry Pi 5 hardware uncovered two packaging
  bugs: the AppImage lacked its own `AppRun` entry point and invoked the
  bundled Python interpreter with no arguments, landing in an interactive
  console instead of the GUI (now fixed via a dedicated `entrypoint.sh`
  recipe); additionally, the pinned PyQt6/PyQt6-Qt6 versions required a
  newer glibc (2.39) on aarch64 than current Raspberry Pi OS/Debian 12
  "bookworm" ships (2.36), failing there with a `GLIBC` load error.
...
- **Hybrid security scan model: CodeQL automatic, Codex manual only
  ...
  push/PR to `main` and weekly, providing deterministic, GitHub-native SAST
  baseline coverage without depending on external API quota. The existing
  Codex Security Scan now runs exclusively via manual `workflow_dispatch`;
  the previous 14-day cadence path (anchor date, `cadence` job) and its
  enable/disable switches are removed as redundant for a purely manual
  trigger. Prompt, JSON schema, artifact upload, findings validation,
  deduplicating issue sync and permission separation (`issues: write` only
  in the downstream sync job) remain unchanged. `SECURITY.md` now describes
  the full, currently active security-check landscape; the architecture
  decision is documented as an ADR
...
- **New app icon: "Liquid Glass" design (#667).** The shared master icon
  `BgRemover_icon.png` (1024×1024 RGBA, transparent outer corners) was
  replaced; the macOS `.icns`, Linux AppImage, and `.deb` icon all derive from
  the same repository master, so every distribution path shows the new
  design. Linux packaging tests now also check icon dimensions and the alpha
  channel.
...
- **Update-check core logic (#564, part of Epic #563).** New Qt-free module
  `app_update.py`: `check_for_update(current_version)` queries the GitHub
  Releases API (stdlib `urllib.request` only, no asset download) and returns a
  structured `UpdateCheckResult` (`UP_TO_DATE` / `UPDATE_AVAILABLE` /
  `CHECK_FAILED`); every network/parsing error is returned as `CHECK_FAILED`
  with a readable message, never as an exception.
- **AI model status detection (#568, part of Epic #563).** New Qt-free module
  `ai_model_status.py`: `get_model_status()` detects whether the rembg default
