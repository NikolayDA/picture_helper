...
- **L'export EufyMake écrit la résolution du projet en `pHYs` PNG (#689, #691).**
  `color_motif.png`, `height_map.png` et `gloss_mask.png` portent désormais les
  DPI X et Y dérivés de la taille physique du projet comme valeurs `pHYs`
  séparées (pixels par mètre entiers). D'après les observations, eufyMake
  Studio 4.2.2 utilise cette valeur comme taille de départ ; sans `pHYs`, il
...
- **L'aperçu 3D annonçait « prêt » alors que l'environnement ne pouvait rendre
  ...
  « indisponible » avec explication et « Réessayer ». La sonde effectue
  désormais une preuve de rendu minimale : un objet framebuffer de 4 × 4
  exactement du type que `QOpenGLWidget` crée pour son framebuffer de widget
  (`CombinedDepthStencil`) est créé, lié et effacé. Comme il s'agit
  exactement de la séquence du visualiseur, la preuve ne peut jamais être plus
  stricte que le visualiseur lui-même : le matériel fonctionnel n'est pas
  exclu. La sonde du préflight de recette (`scripts/qt_gl_probe.py`) reprend la
  même règle sous son étape existante `kontext` ; la provenance du moteur de
  rendu est désormais journalisée aussi en cas d'échec. Le texte
  « indisponible » nomme maintenant le résultat (« ne peut pas rendre en
  OpenGL 2.1 ») au lieu d'une seule des deux causes possibles.
...
- **L'AppImage Linux redémarre ; compatibilité aarch64 restaurée (#595).**
  Des tests manuels sur du matériel réel Raspberry Pi 5 ont révélé deux bugs
  d'empaquetage : l'AppImage n'avait pas son propre point d'entrée `AppRun`
  et invoquait l'interpréteur Python embarqué sans argument, aboutissant à
  une console interactive au lieu de l'interface graphique (corrigé via une
  recette `entrypoint.sh` dédiée) ; de plus, les versions figées de
  PyQt6/PyQt6-Qt6 exigeaient sur aarch64 une glibc plus récente (2.39) que
  celle fournie par l'actuel Raspberry Pi OS/Debian 12 « bookworm » (2.36),
...
- **Noms des artefacts de release désormais univoques par
  plateforme/appareil (#584).** Les cinq téléchargements de release
  (AppImage/`.deb` pour Linux x86_64 et Linux/Raspberry Pi aarch64, `.dmg`
  pour macOS arm64) s'appellent désormais
  `BgRemover-X.Y.Z-<tag-plateforme>[-ai].<extension>` au lieu de la simple
  architecture `uname` – p. ex.
  `BgRemover-2.6.0-linux-raspberrypi-arm64-ai.AppImage`. Auparavant, le
  `.deb` Linux/Raspberry Pi et le `.dmg` macOS portaient tous deux la même
  étiquette `arm64` et ne se distinguaient que par l'extension ; le suffixe
  `-ai` rend en outre visible que chaque artefact – y compris le build
  Raspberry Pi – embarque la même suppression d'arrière-plan par IA intégrée
  que le build macOS.
