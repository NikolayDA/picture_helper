...
- **O5 🟢 — Smoke d'interaction (qtbot) plus tôt dans la CI.** Le smoke de démarrage de l'app (`test_app_smoke.py`) et les tests MainWindow/widgets tournent déjà dans le gate de PR ; seule la suite d'interaction pilotée par qtbot (`test_ui_interactions.py`) est limitée au nightly via `-m 'not ui'`. Tirer un petit sous-ensemble explicitement marqué (p. ex. un marqueur `ui_smoke`) vers PR/Full CI ; la suite qtbot complète reste nightly.
