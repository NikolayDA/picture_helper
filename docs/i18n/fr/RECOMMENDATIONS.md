...
**EufyMake #681/#687–#691 :** la PR #948 est fusionnée et #689 fournit 36 fixtures individuels plus un vrai paquet d'export de quatre fichiers. Les DPI X/Y séparés, les valeurs manifeste/`pHYs` contradictoires et les repères COLOR/HEIGHT/GLOSS pixel à pixel sont couverts automatiquement. Studio 4.2.2 confirme le repli à 72 DPI, la priorité `pHYs` par axe, l'absence de prise en charge du JSON dans l'import d'image, la priorité de la taille manuelle, la rotation et le recadrage d'une image. I-06 est donc observé ; #687 atteint 17/18 critères et n'attend plus que la revue finale après les tests réels. L'usage complet des 16 bits, les dimensions physiques imprimées, la conversion gris→mm et l'intensité du gloss restent des questions matérielles de #688–#690.
...
| [#690](https://github.com/NikolayDA/picture_helper/issues/690) | Valider la sémantique gloss/vernis | 🟡 Moyenne (gloss déjà marqué « expérimental » dans le code) | 🔴 Élevée (impressions physiques, consommation de matériau) | – (aucun agent ; matériel réel requis) | Bloqué (externe) + préparation partielle : Alpha/couverture et repères COLOR/HEIGHT/GLOSS pixel à pixel existent ; restent les dimensions gloss supplémentaires, les cellules d'intensité/polarité et les observations réelles d'aperçu/impression |
