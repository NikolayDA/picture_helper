...
1. **#595** — tous les points de la liste « toujours ouvert » sont satisfaits ; la ligne x86_64 reste « en pause/déclarée ouverte », mais selon une décision explicite, cela ne bloque pas la fermeture (à l'image du propre critère d'acceptation de #639).
...
De plus, deux nouveaux tickets issus d'audits automatisés sont apparus depuis le dernier tour (**#659**, **#660**), encore sans décision.
...
- **#659/#660 sont nouveaux et toujours non tranchés :** #660 dispose déjà d'un correctif terminé mais non fusionné sur la branche `claude/festive-gates-4dkzds` (commit `80b7aa0`) ; #659 est une analyse pure sans changement de code, proposant deux nouveaux identifiants de constat (**N9**/**O8**), encore en attente de validation du propriétaire.
