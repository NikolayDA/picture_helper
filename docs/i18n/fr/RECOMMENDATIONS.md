...
Maintenant **trois** issues ouvertes. **#176 est résolue** (qualité de code
via la PR #198, tests `app.py`/`main_window.py` via la PR #214),
**#166/#178/#185** de même (docstrings via la PR #219, caviardage du
diagnostic via la PR #220, découplage des tests via la PR #221), **#205/#206
sont closes** (épinglages verrouillés par tests via la PR #222) ;
#199/#200/#201/#202 étaient déjà résolues (PR #215/#209/#211). Du lot
`pip-audit` du 2026-06-07 (#200–#206) ne restent ouverts que les points de
veille #203/#204 ; #195 reste clos et vérifié.
...
4. **#176 faite (PR #198/#214)** — `E741` restreint, `check_untyped_defs` activé progressivement, message d'état pour cancel_ai, `shutdown_all` annule les références de threads ; tests dédiés pour `app.py`/`main_window.py`.
