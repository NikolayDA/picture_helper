...
- **O2 🟢 — App Linux / empaquetage.** ✅ Terminé (PR 5 + PR 6) : **AppImage** portable plus un deuxième format **.deb** (fichier desktop, icône, métadonnées AppStream) ; un **workflow de release** construit les deux pour **x86_64 et aarch64/Raspberry Pi OS** sur des runners natifs et les attache à la release. Les smoke tests gardent métadonnées et workflow cohérents — abaisse la barrière d'entrée comme le bundle `.app` de macOS.
...
- **PR 5 — Base du packaging Linux.** ✅ Terminé. AppImage comme artefact cible : `packaging/linux` avec un `.desktop` Freedesktop, une métainfo AppStream et un script de build `python-appimage` ; id d'app `de.bgremover.app` (identique au bundle macOS), `app.py` appelle `setDesktopFileName` ; smoke test autonome (cohérence desktop/AppStream/pyproject).
- **PR 6 — Extension du packaging Linux.** ✅ Terminé. Deuxième format `.deb` (encapsule l'AppImage → apt install + intégration au menu), variante aarch64/Raspberry Pi OS et un workflow de release GitHub Actions (AppImage + `.deb` pour x86_64 et aarch64, attachés à la release) ; le smoke test construit réellement un `.deb` et vérifie le workflow.
