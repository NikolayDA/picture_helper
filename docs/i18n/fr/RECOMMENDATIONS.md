...
## État actuel (2026-07-23, #668/#669 clos)

Ruff, mypy et la suite de tests locale restent la base avant tout nouveau PR. Depuis le dernier tour, les deux tickets d'audit encore ouverts ont été clos :

- **#669** (état en direct obsolète) — fermé directement : la conciliation RECOMMENDATIONS précédente (PR #671) avait déjà entièrement mis le contenu à jour (#659/#660 correctement listés comme fermés, #668 déjà inclus) ; un PR supplémentaire sans changement n'était pas nécessaire.
- **#668** (références au jeu de captures obsolètes) — corrigé par cette mise à jour : `ANLEITUNG.md` et `README.md` (chacun avec ses cinq traductions) pointent désormais vers le jeu actuel `bgremover_complete_20260722_171622`. `docs/history/EPIC-582-ABNAHME.md` conserve délibérément le jeu du 2026-07-19 — il s'agit là de la véritable preuve d'acceptation (provenance GPU/OS/rendu consignée dans le manifeste), et le faire basculer aurait faussé la preuve documentée ; une note explicative a été ajoutée et l'ancien répertoire est conservé en conséquence. Un nouveau test de gouvernance (`tests/test_screenshot_references.py::test_docs_reference_latest_screenshot_set`) détectera désormais une répétition de cette dérive silencieuse.

État en direct : **2** tickets ouverts (#656, #245) — tous deux purement externes/opérationnels, aucun blocage de code.
...
3. Le release v2.7.0 est entièrement publié — aucune autre étape liée au release n'est nécessaire.
