...
- **"从图像生成高度图"按钮置顶显示于步骤 5 最上方。** 高度选项卡的主按钮现名为
  "从图像生成高度图"（原为"从图像生成"），不再包裹在原先的"获取"卡片中，
  直接置于步骤 5 右栏最顶部（位于图层卡片和 3D 区域之前）；"导入灰度图…"紧随其下
  （仍仅限专家模式）。

- **更紧凑的检查器头部：模式提示改为工具提示，不再常驻显示。** 检查器头部下方那行常驻显示、用于说明当前标准/专家模式的提示文字已移除。现在当鼠标指针悬停在标准/专家切换开关上或该开关获得键盘焦点时，说明会以工具提示的形式出现（并作为无障碍描述继续提供给屏幕阅读器）——右侧边栏由此为真正的工具腾出了空间。
...
- **影响：** 功能版本。悬浮缩放药丸现在同样控制 3D 浮雕预览；此外还有四项
  修复（切换模式时的高度实时预览、运行时的应用图标、两条 macOS 启动路径——后两项仅作用于从源码
  构建的应用包，而非分发的 `.dmg`）以及两处检查器改版。图像、项目与导出数据均保持不变。
...
- **卡片检查器中的标准/专家模式开关（Epic #805，#806–#811）。** 检查器头部的一个
  持久开关（`QSettings`，默认标准模式）在两种视图间全局切换：标准模式每个
  步骤只显示最重要的卡片/选项，专家模式显示完整、不变的全部功能。受影响的
  步骤有「抠图」（画笔大小、选区工具、替换颜色、羽化边缘仅限专家模式）、
  「形状与尺寸」（180°/270°/自由角度、圆角、调整大小）、「浮雕与图层」（图层
  工具栏、角色选择、导入灰度图、编辑/优化卡片）以及「导出」（2D 预览模式、
  UV 打印导出）——「打开」与「调整」保持不变。切换模式不会丢失未保存的值，
  也只重建受影响的卡片，而非整个面板。详见
  [`docs/REDESIGN_SPEC.md`](../../REDESIGN_SPEC.md) §15。

### 变更

- **EufyMake 导出：翻转位深度警告方向，接入打印区域检查（#687）。** 对厂商资料的
  调研（S 级证据，通过搜索引擎提取获得，未与原始资料核对）显示，EufyMake
  为高度图明确建议“16-bit/channel if the option is available”——此前校验器
  恰恰将 16 位路径标记为未确认，而 8 位则悄无声息地通过。现在
  `BIT_DEPTH_UNCONFIRMED` 在 8 位时触发，不再在 16 位时触发。新增的检查项
  （`PRINT_AREA_EXCEEDED`，警告级）还会将图案的物理尺寸与 eufyMake 标准平板
  （330 × 420 mm）进行比对——此前底层的通用打印区域检查在生产路径中从未
  真正运行过，因为没有调用方传入目标介质。位深度与物理尺寸警告文案也做了
  精确化处理（#797）：现在明确指出厂商建议所指的 ZBrush/Photoshop 导出
  工作流以及尚未确认的 Studio 导入约定，而不是暗示一项普遍强制要求。

### 本版本说明

- **影响：** 功能版本。在卡片检查器中新增了全局标准/专家模式开关（按步骤
  渐进呈现，见上文），并精确化了两条 EufyMake 导出警告文案；图像、项目或
  导出行为本身不受影响。
- **受影响的用户：** 所有用户从首次启动起都会在检查器头部看到新开关（默认：
  标准模式，因此以往行为在视觉上保持不变）；使用带高度图的 EufyMake 导出的
  用户会看到更精确的警告文案。
- **升级相关性：** 建议所有用户升级。无需任何迁移步骤——项目文件
  （`.bgrproj`）、导出格式和设置均保持兼容；可在不改变数据的情况下降级到
  2.7.3（新增的、追加式的 `expert_mode` 设置键会被旧版本直接忽略）。
- **支持的平台：** macOS arm64（`.dmg`）、Linux x86_64 和 Linux arm64（均提供
  `.AppImage` 和 `.deb`），Python ≥ 3.10。所有构建产物均捆绑 AI 后端
  （`-ai` 后缀）。macOS 应用为临时签名，未使用 Developer ID 公证——首次启动
  需右键点击 →「打开」。
...
- **受影响的用户：** 使用「最近打开」列表并打开来自不受信任来源文件的用户——
...
- **发布验收：补齐了四项此前仅部分获得证据的检查（#686）。** 已发布版本的构件
  现在通过 `browser_download_url` **不带**访问令牌下载，也就是与用户从公开发布
  页面下载完全相同的路径；因此意外保持为私有的发布会被立即发现。此外，打包构件
  内的自动化钩子会将窗口标题中显示的产品版本与构件文件名推导出的预期值进行核对，
  并通过真正的 `save_project` 路径保存一份受控的项目副本、重新加载并逐项比对。
  验收证据现在还会逐个构件记录：其 SHA256 是针对提供方摘要独立确认过的，还是仅仅
  计算得出的。

### 关于此版本的说明

- **影响：** 目前仅涉及发布验收工具的变更。程序本身没有任何变化——没有新功能，
  图像、项目和导出行为均未改变。
- **受影响的用户：** 日常使用中无人受影响。这些变更涉及在发布前于真实硬件上
  验证发布构件的工具。
- **升级相关性：** 不急。项目文件（`.bgrproj`）、导出格式和设置保持兼容；同样
  可以在不改变数据的情况下降级回 2.7.1。
- **支持的平台：** macOS arm64（`.dmg`）、Linux x86_64 与 Linux arm64（各含
  `.AppImage` 和 `.deb`），Python ≥ 3.10。所有构件均捆绑 AI 后端（`-ai` 后缀）。
  macOS 应用为临时签名（ad-hoc），未使用 Developer ID 进行公证——首次启动请
  右键点击 →“打开”。
- **已知限制：** 相较 2.7.1 没有新的限制。
...
- **3D 浮雕预览：重复上传网格时 GPU 缓冲区/VAO 会不断累积（PR #676）。** 每次在
  ...
  3D 显示，不涉及图像、项目或导出数据。
- **3D 浮雕预览：GPU 缓冲区创建静默失败却被当作成功（#711）。** 在有问题的驱动上，
  当 `QOpenGLBuffer.create()` 或 `bind()` 返回 `false` 时，上传仍会继续——尽管
  从未创建出任何缓冲区，预览却报告自己「就绪」。现在两个返回值都会被检查：上传
  失败时会立即且仅一次释放已创建的缓冲区和顶点数组对象，并将预览切换到已知的
  错误状态并回退到 2D，而不是显示空白的 3D 视图。

### 关于此版本的说明

- **影响：** 纯补丁版本。仅修复上述可选 3D 浮雕预览中的 GPU 资源泄漏；没有新增
  功能，也不改变图像、项目或导出行为。
- **受影响的用户：** 仅使用 3D 浮雕预览的用户（工作流步骤“浮雕”、分段“显示
  [3D]”或“视图 → 显示 3D 浮雕”）。该问题在长时间会话中反复切换 2D↔3D 时可见：
  GPU 内存占用持续增长。仅使用 2D 预览的用户不受影响。
- **升级相关性：** 建议使用 3D 预览的用户升级，其他情况可选。无需任何迁移步骤
  ——项目文件（`.bgrproj`）、导出格式和设置保持兼容；降级回 2.7.0 同样不会改变
  数据。
- **已知限制：** 相较 2.7.0 没有新的限制。该修复通过针对伪资源的无 GL 回归测试
  得到保障；在真实 OpenGL 上下文中进行长会话内存测量的工作单独记录在子议题
  #684 中，不属于此补丁版本。
...
- **Qt 插件暂存现在会可靠清理每个实例的临时目录（#631 后续）。** 每个进程使用
  ...
  AppImage 布局遗留的孤立目录会在其 `Qt6/lib` 链接失效时被安全删除。
...
- **全新应用图标:「Liquid Glass」设计(#667)。** 共享的主图标
  `BgRemover_icon.png`(1024×1024 RGBA,外角透明)已被替换;macOS 的
  `.icns`、Linux 的 AppImage 和 `.deb` 图标均派生自同一个仓库主文件,因此
  每条分发路径都会显示新设计。Linux 打包测试现在还会检查图标的尺寸和
  透明通道。
