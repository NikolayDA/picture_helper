...
- **深色主题背景色与原型对齐。** 深色模式的背景区域(`theme.DARK`:检查器面板、
  步骤条、工具栏、导航底栏、状态/菜单栏、控件与卡片)现在采用已确认原型
  (`design/Prototyp A - Geführter Workflow.dc.html`)中偏冷的蓝灰色调,而不是
  接近纯黑的中性色。`card_bg` 特意保留得比原型数值更深一档,以确保卡片上的
  `text3` 仍满足 ≥ 4.5:1 的 WCAG AA 对比度约定(#441);`docs/REDESIGN_SPEC.md`
  §2 记录了新数值以及这一处刻意的偏差(#475)。
