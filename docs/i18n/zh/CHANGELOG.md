...
- **在无法渲染任何画面的环境中，3D 预览仍报告「就绪」(#1002)。**
  ...
  「不可用」状态及其说明和「重试」。现在探测会额外执行一次最小渲染验证：创建、绑定
  并清除一个 4 × 4 帧缓冲对象，其类型与 `QOpenGLWidget` 为自身控件帧缓冲所创建的
  完全一致（`CombinedDepthStencil`）。由于这与查看器自身的调用顺序完全相同，该验证
  永远不会比查看器本身更严格——可用硬件不会被误判排除。验收 preflight 的运行器探测
  (`scripts/qt_gl_probe.py`) 在其既有的 `kontext` 阶段采用同一规则；渲染器来源信息
  现在在失败时也会写入日志。「不可用」状态的文案现在陈述结果（「无法使用 OpenGL 2.1
  渲染」），而不再只说明两种可能原因中的一种。
