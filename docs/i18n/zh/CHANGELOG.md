...
- **三维浮雕预览在绘制期间若有另一个三维查看器被释放，可能导致进程崩溃（#1024）。**
  Qt 在绘制前将 GL 上下文设为当前，并在绘制后不加检查地使用它。若垃圾回收在绘制中途
  释放了一个已废弃的三维查看器，其销毁过程会清除当前上下文，应用随即因段错误而终止，
  而不是给出提示。查看器现在会在绘制后自行恢复其上下文。该问题在会话平台（`xcb`）下的
  完整测试运行中被发现；原因在于对象生命周期，与驱动无关。
- **三维浮雕预览可能被误判为错误状态（#1023）。** 预览的渲染证明通过一个短定时器报告缺失的
  部件帧缓冲。若显卡驱动在此之后才交付第一帧，已排队的错误仍会发出：三维视图显示
  错误页面（“Qt 未持有部件帧缓冲”），尽管它刚刚渲染成功。图形上下文切换后同样如此。
  成功的一帧和上下文切换现在会撤回该报告；真正的故障情形仍会被识别。
- **在无法渲染任何画面的环境中，3D 预览仍报告「就绪」(#1002)。**
  `probe_3d_capability` 此前只检查上下文层面的事实：上下文创建、当前的离屏表面、
  非纯 OpenGL ES 上下文以及 GL 2.1 函数集。在 Raspberry Pi 5（Debian 13
  「Trixie」，Broadcom V3D + Mesa）上这四项全部通过，但 `QOpenGLWidget` 同时记录
  `No fbo, cannot render`——3D 标签页切换为「就绪」后保持空白，而不是显示已文档化的
  「不可用」状态及其说明和「重试」。根因在于 Qt 平台而非驱动：`QOpenGLWidget` 需要
  支持控件渲染的平台集成，而 `offscreen`、`minimal` 和 `vnc` 均不具备，Qt 自身在那里
  就声明该控件不受支持。探测现在会**首先**拒绝这些平台，且在任何 GL 调用之前。此外
  它还执行一次最小渲染验证：创建、绑定并清除一个帧缓冲对象，其类型与最小尺寸与
  `QOpenGLWidget` 为自身控件帧缓冲所创建的完全一致，并在之后检查 GL 错误码——这覆盖
  的是另一类问题：在真实会话
  中无法提供完整渲染目标的驱动。两条规则都不会在可用硬件上关闭 3D；存疑时 3D 保持开启。
  验收 preflight 的运行器探测 (`scripts/qt_gl_probe.py`) 在其既有的 `kontext` 阶段
  采用该渲染验证；渲染器来源信息现在在失败时也会写入日志。「不可用」状态的文案现在
  陈述结果（「无法使用 OpenGL 2.1 渲染」），而不再只说明其中一种可能原因。顺带修复：
  在没有运行中的 `QGuiApplication` 时，探测会以 SIGSEGV 崩溃，而不是报告原因。
- **当 Qt 拒绝出帧时，3D 预览保持空白（#1004）。** #1002 的前置检查是必要
  条件，但并不充分：若因窗口生命周期原因缺少控件帧缓冲，任何探测都看不到。此时
  Qt 以日志警告而非错误拒绝出帧——3D 标签页报告“就绪”却显示空白画面。现在查看器
  自行证明其帧：绘制时若发现 Qt 未持有帧缓冲，在连续三次拒绝后切换到错误状态，
  并给出说明和“显示 2D 浮雕”。该规则不会误判正常硬件——只要 Qt 确认过一帧，查看
  器即永久视为正常；从未被绘制的查看器不作评判，单次拒绝也不足以判定。
