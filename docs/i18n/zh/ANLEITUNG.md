...
- **编辑** —— 撤销/重做、旋转（向左/向右 90°）、水平/垂直翻转，以及取消选区/
  反选和*还原原图*。当你更习惯使用菜单而非工具栏或选项卡时很方便。
- **视图** —— *适应窗口*（⌘0）；参见下文"缩放与视图"。
...
打开时支持 PNG、JPEG、WebP、TIFF、BMP 和 GIF 等常见格式；保存为 PNG、JPEG、WebP 或 TIFF。大型图片在后台加载 ——
状态栏显示进度。
...
右侧面板的第一个选项卡控制选区行为；它在上方的界面概览（[第 2 节](#2-程序界面概览)）和[第 6 节](#6-创建选区)的
插图中已经可以看到。
...
- **彩色图案**（必选）：来自彩色合成的 RGBA PNG。
- **高度图**（可选，仅在存在高度图层时）：灰度，**浅色 = 高，深色 = 低**。
- **光泽蒙版**（可选，实验性）：辅助资源。

对话框会在写入**之前**检查项目：错误会阻止导出，警告需要明确确认。随后在 EufyMake
Studio 中导入并定位这些资源，在那里分配墨水模式/图层，并将 Studio 项目另存为
`.empf`。
