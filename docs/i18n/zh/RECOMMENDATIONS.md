...
| [#1045](https://github.com/NikolayDA/picture_helper/issues/1045) | CHANGELOG.md：缺少 PR 引用（#1023）；历史小节标题 | 🟢 低（文档准确性——不过发布正文正是由该文件生成） | 🟢 低（六个版本各一行） | Sonnet，低 | Ready for PR：第 1 点在下次候选构建前处理；第 2 点有意作为历史冻结保留 |
...
| [#1032](https://github.com/NikolayDA/picture_helper/issues/1032) | [史诗] 流程瘦身：分诊迁至 GitHub，减少漂移义务 | 🟠 高（测量窗口内 156 个主线提交中有 27 个纯属分诊维护） | 🔴 高（十一个工作包 #1033–#1043，含顺序与依赖） | –（史诗） | 进行中：顺序为 #1033 → #1040（+#1042）→ #1041/#1043；#1031 以优先级 0 排在最前 |
...
2. **#1045** 与 **#1044**：两个小而边界清晰的 PR——六个 CHANGELOG 版本中缺失的 `#1023` 引用，以及
   `tests/test_preview3d_controller.py` 中 #1004/#1005 的测试缺口。
3. **启动 v2.10.0**：范围已在 `[Unreleased]` 中；在 #1045 之后，通过
