...
| [#1004](https://github.com/NikolayDA/picture_helper/issues/1004) | 3D 查看器在从未产生任何画面的情况下报告「就绪」——查看器侧缺少渲染证明 | 🟡 中（#1002 的遗留情形；影响处于会话平台但控件帧缓冲损坏的设备） | 🟠 高（证据规则必须排除误判：健康但*隐藏*的查看器与损坏者测得完全一致） | Opus，高 | 可开始 —— 设计证据规则（`frameSwapped`，仅在 `showEvent` 中启用），让 `has_rendered` 成为 UI 与 `screenshot3d` 的共享来源，并预先规划对三项原生截图准则的影响 |
