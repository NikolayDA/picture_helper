...
#934）。审计覆盖完整合并差异、合并中包含的审查修正及其回归测试。发布引用与
重试路径、安全报告、runner heartbeat/dry run、准备脚手架，以及带来源输出的真实
Qt/GL preflight 均实现一致。未发现具体且可复现的残余问题，因此无需新增后续议题。
已关闭主题不改变下方未结议题清单和三项后续建议。
