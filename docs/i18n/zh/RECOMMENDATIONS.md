...
**EufyMake #681/#687–#691：** PR #951 已合并，#690 将可复现测试集扩展到 41 个独立 fixture 和七个真实导出包。独立 X/Y DPI、清单/`pHYs` 冲突、Gloss 0/128/255、64…192 归一化、尺寸不匹配、Alpha×Gloss 与 HEIGHT×Gloss 均有自动化验证。Studio 4.2.2 现在也确认了 #690 的图像导入边界：每个 PNG 都保持为独立的“Flat”图层，不会自动分配 GLOSS 角色；所有 Gloss/HEIGHT 实体测量仍待完成。#687 达到 17/18，等待真实测试后的收尾评审。
