...
**2026-09-06 复核（状态 `74972f5`）：** 新登记四个议题。#992/#993 来自对整个代码库的死代码分析（ruff、vulture、覆盖全仓库的符号交叉引用）：未使用的导入、局部变量与模块均为零，全部 572 个 `de` i18n 键、24 条状态消息和八个图标资源都有引用。剩下的是一个已被取代的前身（`has_blocking_gaps`）、一个无人读取的契约常量（`qt_gl_probe.STAGES`），以及没有消费者的样式/模型 API。#994 报告了 `PyQt6-Qt6==6.7.3` 固定版本中两个 #762 未列出的 Qt SVG CVE：新的是风险类别（use-after-free 而非仅导致崩溃的 DoS），而非攻击路径或可选对策；aarch64 wheel 的瓶颈依旧。#995 正是关于本表漂移的 CI 发现。无新的产品缺陷。
