...
**发布评估：暂不需要新版本。** 自 `v2.9.0`（2026-08-29）以来共有 25 个主线提交，全部属于发布自动化、文档与治理；`[Unreleased]` 为空，`bgremover/` 包内仅证据钩子 `update_check_probe.py`（#917）有变动。构建候选版本对用户不会带来任何可见内容。未来 **v2.10.0** 的预期范围：基于现已批准的 ADR #692 的 COLOR 色调/灰度引擎（史诗 #682 的 #693/#694），可选再加上 #949。

**EufyMake #681/#687–#691：** PR #948 已合并，#689 现包含 36 个独立 fixture 和一个真实的四文件导出包。独立 X/Y DPI、冲突的清单/`pHYs` 数值，以及逐像素一致的 COLOR/HEIGHT/GLOSS 定位标记均有自动化验证。Studio 4.2.2 已确认无 `pHYs` 时回退到 72 DPI、按轴采用 `pHYs`、图像导入不支持 JSON、手动尺寸优先，以及旋转和单图裁剪。I-06 因此已观察；#687 达到 17/18，只待真实测试后的收尾评审。完整 16 位使用、实体打印尺寸、灰度到毫米映射和 gloss 强度仍是 #688–#690 的硬件问题。
...
|---|------|--------|--------|--------------------|--------|
| [#681](https://github.com/NikolayDA/picture_helper/issues/681) | [Epic] EufyMake 目标配置文件 —— 验证 Height/Gloss/mm-DPI | 🟠 高（关系到最重要导出目标的正确性） | 🔴 高（5 个子议题，需要物理硬件） | –（Epic） | #687 已完成 17/18 项；仅剩真实测试后的收尾评审，#691 等待 #688–#690 |
...
| [#690](https://github.com/NikolayDA/picture_helper/issues/690) | 验证 gloss/亮光漆语义 | 🟡 中（代码中 gloss 已标记为“experimental”） | 🔴 高（需物理打印、消耗材料） | –（无需 Agent；需要真实硬件） | 阻塞（外部）+ 前期工作部分完成——已有 Alpha/覆盖度及逐像素一致的 COLOR/HEIGHT/GLOSS 定位标记；仍缺额外 gloss 尺寸、强度/极性测试单元和真实预览/打印观察 |
