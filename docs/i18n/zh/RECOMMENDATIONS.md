...
**EufyMake #681/#687–#691：** PR #948 已合并，#689 的准备工作现包含 36 个独立 fixture 和一个真实的四文件导出包。独立 X/Y DPI、故意冲突的清单/`pHYs` 数值，以及逐像素一致的 COLOR/HEIGHT/GLOSS 定位标记均有自动化验证。Studio 4.2.2 的两条路径已记录且未打印。#687 仍为 16/18：I-06 尚需真实 Studio 观察，随后进行收尾评审。完整 16 位使用、尺寸优先级、实体毫米尺寸、灰度到毫米映射和 gloss 强度仍是 #688–#690 的硬件问题。
...
| [#689](https://github.com/NikolayDA/picture_helper/issues/689) | 验证 mm/DPI、目标尺寸、定位契约 | 🟠 高（打印尺寸/对位） | 🔴 高（物理测量、对照图案） | –（无需 Agent；需要真实硬件） | 进行中：仓库准备已完成，36 个 fixture、真实导出清单、X/Y DPI 和 COLOR/HEIGHT/GLOSS 对位均已验证。受控 Studio 导入、优先级/舍入观察和物理测量仍待完成 |
...
3. 在 Studio 中受控导入 **#689**；然后仅在设备/材料获批后，与 #687 的剩余部分、
   #688 和 #690 一并测量。
