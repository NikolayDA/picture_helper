...
**EufyMake #681/#687–#691：** PR #951 已合并，#690 将可复现测试集扩展到 41 个独立 fixture 和七个真实导出包。独立 X/Y DPI、清单/`pHYs` 冲突、Gloss 0/128/255、64…192 归一化、尺寸不匹配、Alpha×Gloss 与 HEIGHT×Gloss 均有自动化验证。Studio 4.2.2 已确认 #689 的导入契约；新的 #690 导入及所有 Gloss/HEIGHT 实体测量仍待完成。#687 达到 17/18，等待真实测试后的收尾评审。
...
| [#690](https://github.com/NikolayDA/picture_helper/issues/690) | 验证 gloss/亮光漆语义 | 🟡 中（代码中 gloss 已标记为“experimental”） | 🔴 高（需物理打印、消耗材料） | –（无需 Agent；需要真实硬件） | 阻塞（外部）+ 准备工作已完成——模式 4 覆盖缺失/0/128/255、受限渐变、尺寸、Alpha×Gloss、HEIGHT×Gloss 与配准；仍待 Studio 导入和实体 gloss 证据 |
