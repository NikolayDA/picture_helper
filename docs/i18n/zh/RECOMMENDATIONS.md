...
**发布评估：尚未启动候选版本。** 自 `v2.9.0`（2026-08-29）以来共有 31 个主线提交。随着 PR #953（版本化 EufyMake 目标配置文件、16 位 HEIGHT 默认值、对话框中的配置文件与 X/Y DPI 显示、清单溯源），`[Unreleased]` 首次包含用户可见的条目；其余均为发布自动化、文档与治理。**v2.10.0** 是仅含 #953 发布，还是与 COLOR 色调引擎（史诗 #682 的 #693/#694，ADR #692）一起发布，由负责人决定。但任何发布前都必须先由 #691 以明确的配置文件版本决策修正快照中的错误证据引用；冻结门禁在 `91b32b4` 状态下为绿色。
...
|---|------|--------|--------|--------------------|--------|
| [#681](https://github.com/NikolayDA/picture_helper/issues/681) | [Epic] EufyMake 目标配置文件 —— 验证 Height/Gloss/mm-DPI | 🟠 高（关系到最重要导出目标的正确性） | 🔴 高（5 个子议题，需要物理硬件） | –（Epic） | #691 已暂定集成；发布前修正错误引用，随后完成 I-08/I-09、#688–#690 和收尾评审 |
| [#687](https://github.com/NikolayDA/picture_helper/issues/687) | 假设清单、厂商资料来源、测试矩阵 | 🟠 高（#688–#691 的约束性基础） | 🔴 高（仓库材料已齐；剩余部分需要真实硬件） | –（无需 Agent；需要真实的 EufyMake 硬件） | 阻塞（外部）—— 已完成 17/18 项；尚缺 I-08（跨角色裁剪/对位）、I-09（`.empf`）及 #688–#690 后的评审 |
...
| [#691](https://github.com/NikolayDA/picture_helper/issues/691) | 将带版本号的目标配置文件整合进 validator/writer/对话框/文档 | 🟠 高（强化生产环境导出路径） | 🟢 发布关键余项较低；🔴 收尾需硬件 | Sonnet，中 + 后续硬件 | 接近完成 —— PR #953 已集成 v1；先以明确版本修正引用，再于 #688–#690 后升级为 `validated` 或创建新版本 |
...
1. **#691**：任何发布前，以明确版本决策和 golden test 修正快照引用。
2. **#693**：ADR #692 已批准；随后依次进行 #694、#695、#696。
3. **#949**：完成四项改动，随后连同平台及可选依赖重新测量基线。
4. **#883**：决定 Qt/代码许可，并证明 `u2net.onnx` 的权利/来源，或选择许可明确的替代模型。
5. 在设备/材料获批后，与 #687 的剩余部分、#688 和 #690 一并完成 **#689** 的物理
