...
**2026-08-31 每日审计：** 已依据合并差异、审查修正和相关测试，复核今日合并的
12 个 PR（#927–#932、#935–#938、#940、#941）及其关闭的议题
（#918–#923、#933、#934）。发布自动化始终保持 fail-closed；其中已覆盖发布
引用、重试路径、安全报告、runner heartbeat/dry run、准备脚手架，以及在成功时
同样输出来源信息的真实 Qt/GL preflight。每项审查发现均在合并前由回归测试闭环。
合并证据记录 2,908 项 pytest 测试；Ruff 在本地通过，而 pytest 因缺少
`libGL.so.1`、mypy 因启用 Python 3.14 而非项目支持版本而无法运行。未发现可证实的残余问题，因此无需新增
后续议题。下方未结议题清单和三项后续建议保持不变。
