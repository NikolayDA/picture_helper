...
## 当前状态（2026-08-01，PR #754 之后的 v2.7.2 工作状态）
...
GitHub 查询后的实时状态：**27** 个未结议题。#742/#743 已由 PR #754 关闭；
#744/#747 正在本 PR 中共同实现。
...
| [#744](https://github.com/NikolayDA/picture_helper/issues/744) | publish 复用已验收的候选产物 | 🟠 高（已验收 ≠ 已发布字节） | 🔴 高（发布路径，难以空跑验证） | Opus，高 | **进行中** —— 本 PR 实现批准清单与独立、无重建、draft-first 的 publish |
| [#747](https://github.com/NikolayDA/picture_helper/issues/747) | 在 #742/#744 之后以机器方式固化新关卡契约 | 🟡 中（避免出现 #709 式的契约漂移） | 🟡 中（扩展现有测试） | Sonnet，中 | **进行中** —— 同一 PR 包含正/负、篡改、重试与字节相等性测试 |
...
1. 在本 PR 中完成 **#744 + #747**；CI 与评审须确认清单、无重建 publish 与篡改测试。
2. 用运行 30706671985 更新 **#680/#685/#686**，并依据剩余判据关闭。
3. 让 **#745/#746** 基于现已确定的 release 契约继续推进。
