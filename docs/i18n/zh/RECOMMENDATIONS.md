...
已确认的质量与稳定性问题分为四个独立实施包：

- **P1 ✅ — 魔棒 / CI 快速修复（N1、N6、N8）。** 在开始切换图片时静默释放魔棒锁，避免后续加载失败导致工具被阻塞。完整 Linux CI 现在也安装 `libgl1`；静态漂移测试保护 Qt 系统包列表。已修正过时的同步拖放路径说明。
- **P2 🟡 — 加固保存流程（N4、N5）。** 显式验证导出扩展名，并通过临时文件加原子替换覆盖现有文件。
- **P3 🟠 — 自适应内存预算（N2、N3）。** 在自由旋转前检查预期大小；统一预算 undo、redo、原始状态和内存密集型操作，尤其针对 Raspberry Pi。
- **P4 🟡 — 解耦包导入（N7）。** 谨慎减少或延迟加载 eager GUI 重导出，使纯逻辑模块无需完整 Qt runtime 即可测试和复用。这不是 bootstrap bug；公共导入必须保持兼容。
