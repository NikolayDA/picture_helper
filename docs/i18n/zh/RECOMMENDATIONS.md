...
开放议题的分诊已在 rail/zoom follow-up 之后更新到当前状态。本 PR 之前，GitHub
显示 14 个开放议题；关闭 **#461** 后，还剩 13 个路线图/Backlog 议题。
...
- **Rail/zoom 波次已完成：** **#455/#456/#457/#458/#463/#464** 已通过 PR #466
  落地，**#465** 有意标记为 `not_planned`。本 follow-up PR 关闭 #466 后续的
  三个 P2（缩放方向、viewport 锚定、高度 dab 预览），并刷新 **#461**。
...
## GitHub 开放议题 — 分诊状态（2026-07-03）

截至 2026-07-03，在此 follow-up 合并前 GitHub 显示 **14** 个开放议题。
**#461** 正是这次快照漂移，将由本 PR 关闭。之后用于规划的开放路线图/Backlog
议题为 **13** 个：右栏润色（**#413/#414**）、i18n/文档
（**#425/#430/#431/#432**）、rollout/发布（**#426/#435/#392/#389**），以及
独立事项 **#299/#318/#245**。

**评论巡查：** PR #466 后续的三个 P2 评论已确认是真实剩余工作，并在本 PR 中处理。
PR #460 的 P2 仍记录为仅限 mockup 的发现 **O8**；真实应用已会自动激活 HEIGHT 图层。

### 合理的组合

- **接近完成的史诗：** #413 仅剩 #414 开放；其令牌已在 `theme.py` 中——补上浅色
  卡片样式后即可关闭。
...
| [#461](https://github.com/NikolayDA/picture_helper/issues/461) | Rail follow-up 后刷新分诊快照 | 🟡 中 | 🟢 低 | **由本 PR 解决** – 将关闭。 |

### 接下来推荐（PR 顺序）

1. 完成 **#414**（浅色卡片样式），随后关闭史诗 **#413**。
