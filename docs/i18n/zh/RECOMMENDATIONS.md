...
## 当前状态（2026-09-07，v2.9.0 已发布，未结议题已全部审计）

**每日审计 2026-09-02（状态 `91b32b4`）：** 已将全部 42 个未结议题与代码、合并记录、评论及 Mac App Store 史诗所涉当前一手资料逐项核对。状态修正已记录于 #681、#688、#691、#682/#693、#914/#918 和 #949。新增实质发现：独立的 `u2net.onnx` 产物并无可靠的 Apache-2.0 证明（#883/#893）；即使应用免费，Apple 仍可能要求 trader 提供付款账户资料（#884/#904）；#895/#899/#906 必须明确选择并测试应用下载验证。无新的产品缺陷或 🔴 发现。

**2026-09-03 复核（状态 `e7c379d`）：** 已将新 PR #955–#957 与未结事项逐项核对。PR #956 在尚未发布的 v1 内有意识地修正了错误证据引用，并增加快照/导出包保护；原 #691 发布关键项至此解决。#955 仅影响测试文档，#957 仅影响发布脚本，均不改变 EufyMake 实证结论。

**2026-09-06 复核（状态 `74972f5`）：** 新登记四个议题。#992/#993 来自对整个代码库的死代码分析（ruff、vulture、覆盖全仓库的符号交叉引用）：未使用的导入、局部变量与模块均为零，全部 572 个 `de` i18n 键、24 条状态消息和八个图标资源都有引用。剩下的是一个已被取代的前身（`has_blocking_gaps`）、一个无人读取的契约常量（`qt_gl_probe.STAGES`），以及没有消费者的样式/模型 API。#994 曾报告 `PyQt6-Qt6==6.7.3` 固定版本中的两个 Qt SVG CVE，现已通过升级到 Qt 6.11 解决：旧固定版本的理由已经过时，因为 Raspberry Pi OS 现已构建 Debian 13，而此次升级关闭了约十条公告而非报告的两条。两项测量佐证：在 6.11 下测试套件仍为 3156 项通过，并且所记录的攻击路径并不存在，因为文件对话框渲染的是 MIME 图标而非文件内容。#995 正是关于本表漂移的 CI 发现。无新的产品缺陷。

**EufyMake 2026-09-07 复核：** 增量配置 v2 现为 Studio 4.3.3、Editor 1.20.0
和固件 4.0.9 的默认配置；配置 v1 仍可作为冻结的 Studio 4.2.2 参考选择。项目
缺少物理尺寸时，v2 会报告 `physical_size_missing`；格式错误的值仍是阻塞错误
`INVALID_TARGET_PARAMS`。GUI 原始导入矩阵已完成 29/29 个测试格，功能表现与
4.2.2 相同。仅 I-06 流程有变化：可选择 `manifest.json`，随后会以
`Unsupported file type.` 拒绝。全部 13/13 个原生项目均可加载；十二个活动项目
使用 `Unidirectional` 进入预览且无警告，项目 03 在 `Retry` 后第二次时间/墨水
估算仍失败。`Bidirectional` 尚未测试。未启动打印：Y 墨水、刮刀和空气过滤器均
已过期，现场有 0.1 mm 黑色纸板。实体 E1 测试仍待完成。

**发布评估：尚未启动候选版本。** 自 `v2.9.0`（2026-08-29）以来，在已审计状态 `74972f5` 共有 58 个主线提交。随着 PR #953（版本化 EufyMake 目标配置文件、16 位 HEIGHT 默认值、对话框中的配置文件与 X/Y DPI 显示、清单溯源）、#971（已确认的平板尺寸）和 #996（项目 DPI 以 PNG `pHYs` 写入导出资源），`[Unreleased]` 已包含用户可见的条目；其余均为发布自动化、文档与治理。**v2.10.0** 是以已累积的 #953/#971/#996 范围发布，还是与 COLOR 色调引擎（史诗 #682 的 #693/#694，ADR #692）一起发布，由负责人决定。PR #956 已以明确保留 v1 的决策及 golden/导出包保护修正错误引用，因此 #691 不再新增发布阻塞；常规发布门禁仍为准。
...
| [#1015](https://github.com/NikolayDA/picture_helper/issues/1015) | `test_a_visible_viewer_proves_its_frame` 不等待 `frameSwapped`（在 Pi 上变红，查看器正常） | 🟡 中（使真实硬件上的反向验证变红，尽管 #1004 的渲染证明在那里可用） | 🟢 低（用有界等待替代固定循环，在 Pi 上复测） | Sonnet，小 + 硬件 | 用 `qtbot.waitSignal(viewer.frameSwapped, timeout=2000)` 等待，负向对照等待同样时长；在 Pi 上重跑 TESTING.md 第 2 步 |
| [#1016](https://github.com/NikolayDA/picture_helper/issues/1016) | `[test]` extra 缺少 PyYAML——59 个 workflow 守卫静默跳过（CI 和 Pi） | 🟠 高（release/heartbeat workflow 的守卫在 CI 中从未真正测量） | 🟢 低（extra + constraints 锁定，更新许可证报告，用硬导入替代 `importorskip`） | Sonnet，小 | 将 `pyyaml` 加入 `[test]` 与 constraints，重新生成 `LICENSES.md` ×6，将 `importorskip("yaml")` 换为硬导入，由 `make doctor` 检查该模块 |
...
3. 在设备/材料获批后，与 #687 的剩余部分、#688 和 #690 一并完成 **#689** 的物理
   测量；原生 HEIGHT/Gloss 路径和 I-08 已预检。之后复核配置 v2 的证据状态；v1
   保持冻结，新增或矛盾语义应使用后续配置版本。
