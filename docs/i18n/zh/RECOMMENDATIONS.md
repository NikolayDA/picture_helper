...
## 当前状态（2026-08-28，v2.9.0 候选版正在验收，未结议题已全部审计）
...
| [#881](https://github.com/NikolayDA/picture_helper/issues/881) | [Release 2.9.0] 验收与发布记录 | 🟠 高（约束性发布关卡） | 🟠 高（硬件验收、来源证明和负责人批准） | –（无需 Agent；需要 runner 与发布/安全负责人） | 阻塞（外部）—— 使 `macos-arm64` 和 `linux-arm64` 上线，执行步骤 5，并记录所有 MUST/负责人决定 |
...
3. **构建并验收 v2.9.0 发布** —— 运行手册第 1–2 步已完成：版本 2.9.0、六种语言的带发布说明的日期化 CHANGELOG 章节、AppStream/许可证快照，以及[范围冻结文档](../../history/RELEASE-2.9.0-scope-freeze.md)（基线 v2.8.0，路径策略 7）。接下来：通过 `release-linux.yml` 构建候选版本，然后在真实硬件上运行 `release-abnahme.yml`。
