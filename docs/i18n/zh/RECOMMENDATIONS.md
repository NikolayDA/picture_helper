...
**需要一个补丁版本：** 与本节先前的版本相反，`[Unreleased]` 实际上**并非**空的——全部六个 CHANGELOG 文件均已包含 #769 的修复（CVE-2025-5683，`_recent_thumbnail_icon` 中的 ICNS 拒绝服务，已通过 PR #782 合并，提交 `b42c738`），因此该安全修复**尚未**进入已发布的 v2.7.2 产物中（PR #785 的 Codex 评审发现）。`pyproject.toml` 仍为 2.7.2。建议：准备补丁版本 **v2.7.3**，专门发布这一项修复（版本号提升 + CHANGELOG 迁移，无需额外代码），随后按发布运行手册执行候选构建。#758（验收/发布协议）与 #748（`UPDATE-01`，真实前版产物证明）已针对 v2.7.2 关闭。
...
1. 准备补丁版本 **v2.7.3**：通过版本号提升 + CHANGELOG 迁移发布已合并的 #769 修复
   （CVE-2025-5683），随后按发布运行手册执行候选构建。
2. 收尾 **#777**：针对 GitHub 真实状态运行 `scripts/recommendations_live_check.py` 直至变绿
   （需要网络访问），随后关闭该议题。
3. 机械式修复 **#773** —— 在全部六个 CHANGELOG 文件中做两行修复，一个小型独立 PR。
4. 启动 **#687** 作为第一个 EufyMake 子议题（先解决格式问题）；**#692**（ADR）与 **#716**
   （测试审计）仍可并行启动。
5. **#781**：现在就实现产物上传；标准评估需等待下一次验收运行。
6. **#741** 在下一个发布周期给出 DoD 证明（publish 后无第二次硬件验收）之前保持未结。
