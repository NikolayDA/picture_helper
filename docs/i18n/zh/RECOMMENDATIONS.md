...
| [#943](https://github.com/NikolayDA/picture_helper/issues/943) | 2026-08-31 审查后续：流程脚本中的五项健壮性问题 | 🟠 高（heartbeat 在未证明就绪时报告 PASS） | 🟡 中（四个脚本，含回归测试的独立修复） | Sonnet，高 | 可启动——先处理 heartbeat 结论，再处理 dispatch 标记、prepare_release（降级/写入顺序）和扫描器 OSError |
