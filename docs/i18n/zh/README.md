...
- **🧊 3D 浮雕预览**：将当前高度图作为可旋转的表面查看，支持旋转、平移、缩放、高度夸张、光照和质量选择；仅影响显示，不改变高度数据或导出。没有 OpenGL 2.1 时，2D 预览仍可使用。
...
*同一步骤的高度区域：从图像生成高度图或导入一张，然后调整——高度语义为
浅色 = 高。*
...
![带显示控件的 3D 浮雕预览](../../../app_screenshots/bgremover_complete_20260828_184550/77_function_preview3d_adjusted.png)
...
5. **浮雕与图层** – 管理项目图层，生成/编辑/优化高度图。存在有效高度图时，可在**显示**中切换 **2D** 和 **3D**；在 3D 中，按住鼠标左键拖动可旋转，使用中键或 `Alt`+拖动可平移，滚轮可缩放。没有 OpenGL 2.1 时，2D 浮雕预览仍可使用。
...
Linux/macOS 矩阵（Python 3.10、3.11、3.12 和 3.13）作为 release 门禁运行：手动候选工作流
会在构建前调用它，之后由独立工作流仅发布已通过硬件验收的字节；此外每周（周日）和手动触发时也会运行。所有本地/CI 测试安装都使用
