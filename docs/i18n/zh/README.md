...
![BgRemover – 带图层面板的主窗口](../../screenshot.png)

*带图层面板（右侧）的主窗口：由彩色图案图层和高度图层组成的项目，并带有角色
分配。*

![高度图工作区](../../screenshot_height.png)

*“高度”选项卡：获取、编辑并优化高度图——灰度视图显示浅色 = 高。*

![2D 预览：颜色上的浮雕与光泽](../../screenshot_preview.png)

*组合式 2D 预览将浮雕和光泽叠加在彩色图案之上——仅为显示，导出不变。*
...
1. 通过 `文件 → 打开`（⌘O）、将图像拖放到窗口中，或以启动路径（CLI / Finder）**打开图像**。
2. 用魔棒、画笔、橡皮擦或多边形套索**进行选区**（标签 *选区*）。
   - `Shift+点击` 添加到选区；`⌘+点击`（macOS）或 `Ctrl+点击`（Linux）从选区中减去。
   - 可用键盘切换工具：`W` 魔棒、`B` 画笔、`E` 橡皮擦、`L` 套索。
3. **编辑背景**（标签 *背景*）：设为透明、替换颜色或平滑边缘——或直接使用工具栏中的 **AI**。
4. **优化与变换**（标签 *调整* 和 *旋转/翻转*）：调整亮度/对比度/饱和度，旋转、镜像翻转、调整尺寸。
5. **形状与裁剪**（标签 *形状*）：圆角处理或按比例裁剪——移动/缩放边框，然后点击 ✓ 应用。
6. **图层、高度与预览**（标签 *图层*、*高度*、*预览*）：管理项目图层，生成/编辑高度图，并以浮雕/光泽检查结果。
7. 通过 `文件 → 保存`（⌘S）**保存**为 PNG、JPEG、WebP 或 TIFF——或 `项目 → 导出 EufyMake Studio 资源…` 用于 UV 打印。
...
- **`MainWindow`** 构建工具栏和状态/裁剪栏，并连接画布、菜单、
  右侧面板和 worker。
- **`right_panel`** 基于一组回调构建右侧八个标签页（预览、选区、
  背景、调整、旋转/翻转、形状、图层、高度）；`project_model`/`height_map`
  提供与 Qt 无关的图层与高度模型。
