...
Станом на 2026-06-22 GitHub показує **11** відкритих issues. Критичну регресію
експорту **#363** виправлено через **PR #367** і закрито; kind/role-знахідку
Height Map **#364** реалізовано та закрито цим PR (центральний контракт
`HEIGHT_MAP`↔`HEIGHT` у моделі, персистентності, UI та canvas із безвтратною
нормалізацією legacy). Лишаються **епік експорту EufyMake #351** і sub-issues
**#352–#355** (roadmap-ранг #3), docs-прогалини **#357** та **#339**, наслідок
Height Map **#365** (пам'ять median-фільтра), а також test/CI-знахідки **#318**,
**#299** і **#245**. Шлях підтримки/skip **#322** реалізовано через **#342** і
закрито.
...
1. Посилити **#365** до роботи з великими Height Maps через median-preview
   (kind/role-контракт з **#364** уніфіковано цим PR).
2. Потім реалізувати **#352**, спершу ADR; це розблокує #353/#354.
3. Реалізувати **#353** і **#354** паралельно, потім **#355**.
4. Використати **#357**, **#339** і **#299** як нижчі пріоритети.
5. Відкласти **#318** до документації GitHub permission semantics.
6. Лишити **#245** externally blocked (жоден repo-patch не відновить quota).
