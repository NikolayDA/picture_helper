...
1. **#167 Medium** — Double-checked lock у `_ensure_rembg_remove()` + TOCTOU-вікно в `open_validated_image`: реалізовано в **PR #174** (merge очікується; Low-знахідки окремо).
2. **#165** — Виправлення у TESTING.md: низький ризик, добре обмежено.
3. **#163 контент** — Додати відсутні features 2.3.0 + записи `[Unreleased]` у CHANGELOG; git-теги вирішити окремо.
4. **#161 частково** — Видалити жаргон «Runde 5» з опису архітектури в README (виправлення URL клонування потребує рішення про видимість репо).
