...
## Завантажити готовий застосунок (`.dmg`)

Найпростіший спосіб — **без Python, git чи терміналу**: завантажте готовий
пакунок застосунку з [релізів GitHub](https://github.com/NikolayDA/picture_helper/releases).
Для Apple Silicon (arm64) є `BgRemover-<version>-macos-arm64-ai.dmg`
(назва файлу містить платформу й архітектуру); ШІ-вилучення тла вже
вбудоване — це видно із суфікса `-ai`, як і в артефактах для Linux.

Готовий `.dmg` підтримує лише **Apple Silicon (arm64)** і потребує
**macOS 11 (Big Sur) або новішої версії**. `.dmg` для Intel наразі не
збирається; на Intel-Mac використовуйте описаний нижче шлях збирання з коду.

1. Відкрийте `.dmg` і перетягніть `BgRemover.app` до теки **Програми**.
2. Під час **першого** запуску підтвердьте через **правий клік → «Відкрити»** —
   пакунок ще не підписано/нотаризовано Apple, тож інакше Gatekeeper
   попередить про «невідомого розробника».

Як альтернатива — заздалегідь зніміть карантин у терміналі:

```bash
xattr -dr com.apple.quarantine /Applications/BgRemover.app
```

Якщо ви радше збираєте з коду (наприклад, для Intel-Mac чи власних змін),
дотримуйтеся розділів нижче.

## Вимоги

- **macOS** — для готового `.dmg`: macOS 11 (Big Sur) або новіша версія та
  Apple Silicon (arm64)
...
- **`numpy ... incompatible architecture (have 'arm64', need 'x86_64')`**
  ...
  ```
- **Застосунок працює на Apple Silicon, але запускається як `x86_64` під
  Rosetta** → лише `uname -m` не є надійною діагностикою: у трансльованому
  процесі він показує `x86_64`, хоча апаратна архітектура — `arm64`.
  `bash diagnose_mac.sh` тому окремо показує апаратну архітектуру,
  `sysctl.proc_translated`, архітектури бінарних файлів через `file` і
  `platform.machine()` Python застосунку. Якщо `create_BgRemover_app.sh`
  виявляє x86_64-venv на Apple Silicon, він попереджає та пропонує нативну
  перебудову до будь-якого видалення. Еквівалентне ручне відновлення:
  ```bash
  brew install python
  rm -rf "$HOME/Library/Application Support/BgRemover/venv"
  bash create_BgRemover_app.sh
  ```
  Наступний рядок запуску в `bgremover.log` має містити
  `interpreter_arch=arm64` і `proc_translated=0`.
