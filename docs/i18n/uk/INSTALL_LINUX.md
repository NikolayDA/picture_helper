...
`onnxruntime`. Під час першого запуску застосунку з установленим
`rembg` застосунок автоматично одноразово завантажує свою модель
(кілька сотень МБ, кеш у `~/.u2net`). Рядок стану показує
«Завантаження моделі ШІ…», а потім «ШІ готовий». До цього кнопки ШІ
залишаються вимкненими; це очікувана поведінка, а не помилка
встановлення.
...
- **Помилка pip під час встановлення** → в активному venv спершу оновіть
  ...
  python3 -m pip install --upgrade "pip>=26.1.2"
  ```
- **Перший запуск застосунку з ШІ триває довго** → Під час першого
  запуску застосунку з установленим `rembg` застосунок автоматично
  завантажує модель (кілька сотень МБ, одноразово, кеш у `~/.u2net`).
  Рядок стану показує «Завантаження моделі ШІ…», а потім
  «ШІ готовий». До цього кнопки ШІ залишаються вимкненими; це очікувана
  поведінка, а не помилка встановлення.
