...
> Під Linux AppImage і `.deb` є рекомендованими артефактами для кінцевих
> користувачів; прямий запуск із venv залишається задокументованим для
> розробки, тестування гілок і локальних змін.

## Рекомендовано: використовуйте release-артефакти

Для звичайного встановлення під Linux release-артефакти — найзручніший шлях:
**без venv, без pip і без Git-checkout**.

> **Примітка щодо доступності:** Готові артефакти AppImage/`.deb` — **з уже
> вбудованим ШІ** — доступні починаючи з **v2.4.0**. Старіші релізи
> (v2.1.0/v2.2.0) ще не містять таких assets — доки на сторінці релізів немає
> нічого для завантаження, користуйтеся способом через venv/Git нижче.

- **AppImage:** портативний один файл; зробіть його виконуваним і запустіть.
...
Для `.deb` менеджер `apt` розв'язує цю залежність автоматично.
...
> **Системна вимога артефактів (починаючи з наступного випуску):** вбудований
> Qt потребує новішої glibc, ніж раніше, на **обох** архітектурах. Для
> **aarch64** це **glibc 2.39**, тобто Raspberry Pi OS «Trixie» (Debian 13)
> або новіша. Для **x86_64** це **glibc 2.34**, тобто Debian 12,
> Ubuntu 22.04, RHEL 9 або новіші; старіші системи на кшталт Debian 11,
> Ubuntu 20.04 та RHEL 8 випадають. `.deb` оголошує цю межу й відмовляє у
> встановленні, замість падіння під час запуску. На надто старих системах
> лишається шлях через apt/venv нижче.
...
На **Raspberry Pi OS «Bookworm» Desktop** (Debian 12) або новішій
(наприклад, «Trixie»/Debian 13, рекомендовано 64-бітну) встановлення
значно простіше: PyQt6, Pillow та
...
`onnxruntime`. Під час наступного **запуску** застосунку `rembg`
автоматично одноразово завантажує свою модель (кілька сотень МБ, кеш у
`~/.u2net`); рядок стану тим часом показує «Завантаження моделі ШІ…»,
а кнопка ШІ лишається неактивною до завершення — це очікувана поведінка.
Майбутні запуски тоді з venv: `source .venv/bin/activate` і
`python3 -m bgremover`.

Як альтернатива автоматичному завантаженню під час запуску, стан моделі
можна будь-коли перевірити через `Інструменти → Керувати моделлю ШІ…`,
що дозволяє вручну запустити завантаження/повторну спробу (з індикатором
активності та кнопкою скасування) — корисно, якщо початкове завантаження
не вдалося офлайн.
...
- **Помилка pip під час встановлення** → в активному venv спершу оновіть
  ...
  python3 -m pip install --upgrade "pip>=26.1.2"
  ```
- **Кнопка ШІ лишається неактивною одразу після запуску** → Це не
  помилка встановлення: щойно `rembg` встановлено, застосунок
  автоматично одноразово завантажує свою модель під час **запуску
  застосунку** (не при першому кліку ШІ), кілька сотень МБ, кеш у
  `~/.u2net`. Рядок стану тим часом показує «Завантаження моделі ШІ…»,
  а потім «ШІ готовий»; кнопка ШІ лишається неактивною до завершення
  (і в будь-якому разі без завантаженого зображення). Якщо завантаження
  не вдалося офлайн, стан можна будь-коли перевірити через
  `Інструменти → Керувати моделлю ШІ…`, що дозволяє вручну запустити
  завантаження/повторну спробу.
...
- **Raspberry Pi OS «Bookworm» (Pi 4/5) використовує Wayland** → У разі проблем
  ...
  вище).
- **Діагностика помилок** → Точний шлях внутрішнього журналу виконання
  показано в `Інструменти → Налаштування… → Файл журналу`; у Linux він
  лежить у визначеному Qt каталозі під `~/.local/share/`. Під час
  запуску з термінала повідомлення про помилку додатково з'являється
  безпосередньо в консолі.
