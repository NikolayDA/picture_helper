...
Підтримуються поширені формати: PNG, JPEG, WebP, TIFF і BMP. Великі
...
вище (розділ 2) і на ілюстрації в розділі 6.
...
|---|---|---|
| **Допуск (чарівна паличка)** | 0 – 255 | Наскільки схожими мають бути кольори для спільного виділення. **Низько** = лише дуже схожі кольори · **Висо­ко** = багато відтінків. |
| **Розмір пензля** | 4 – 200 пкс | Діаметр пензля та гумки. |

### Дії з виділенням

- **Зняти виділення** – скасовує поточне виділення (також клавішею **Esc**).
...
- **Розширити / Звузити** – збільшує або зменшує виділення на зазначений
  радіус (1 – 20 пкс). Корисно для видалення тонкої кольорової смуги після
  вирізання.
...
4. Під полотном з'являється панель:
...
*Діалог налаштувань: типові теки для відкриття та збереження, бажаний формат
зображення і шлях до файлу журналу з кнопкою «Відкрити теку».*
...
Перші три параметри зберігаються між перезапусками програми.
...
- **Пакет програми** (`BgRemover.app`) є специфічним для macOS; на
  Linux/Windows програма запускається безпосередньо.

---

## 17. Усунення неполадок і файл журналу

У разі проблем перевірте **файл журналу** `bgremover.log`. Він створюється
при запуску програми в специфічній для платформи директорії даних:

- **macOS:** `~/Library/Application Support/BgRemover/`
- **Linux:** `~/.local/share/BgRemover/`

Файл містить повідомлення про стан і деталі помилок (стек-трейси) та є
першим місцем звернення при запитах підтримки.
