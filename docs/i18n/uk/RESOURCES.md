...
| **Pillow** (PIL) | Введення/виведення зображень, EXIF-transpose, обертання/віддзеркалення, маски/альфа, збереження (PNG/JPEG/WebP/TIFF) | `>=10` | 12.3.0 | HPND (також «MIT-CMU»; ліцензія PIL з відкритим кодом) |
...
Завантаження й надалі відбувається неявно під час запуску застосунку
(warmup); стан кешу також можна будь-коли перевірити через `Інструменти →
Керувати моделлю ШІ…`, що дозволяє явно запустити завантаження/повторну
спробу без перезапуску застосунку.
...
працює на Ubuntu + macOS з Python 3.10–3.13 як release-gate (робочий процес
release викликає її на тезі версії перед публікацією), щотижня через cron
або вручну; `ui-nightly.yml` щоночі запускає
...
- **Іконки панелі інструментів/вкладок як PNG**:
  `bgremover/icons/*.png` (`ai`, `bg`, `clear_sel`, `close`, `form`,
  `open`, `restore`, `save`, `transparency`). Завантажуються функцією
  `make_tool_icon()` через `importlib.resources` як дані пакета.
- **Намальовані векторні іконки**: Іконки rail/теми (`move`, `wand`,
